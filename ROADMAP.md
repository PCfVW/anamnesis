...
**Decision coupled to this phase — vendor the ZIP container?** anamnesis delegates ZIP-container parsing (`.npz`, `.pth`) to the `zip` crate, whose `deflate` feature also pulls a DEFLATE *compressor* (`zopfli`) that a read-only consumer never uses. 6.8's ratio cap + aggregate budget want to live *inside* the decompress/read loop, which the `zip` API makes awkward — so this phase is the natural trigger to evaluate vendoring a minimal container reader (the long-standing intent to stop depending on `zip`). Scope it **PTH-first**: PyTorch entries are STORED-only (`build_entry_index` already skips non-`Stored`), so the PTH container needs **zero codec** — the clean pilot. **NPZ** reads DEFLATE today (transparently via `zip`; `np.savez_compressed`), so its vendored form keeps `miniz_oxide` for inflate — **never hand-roll the codec**. Gate the decision on (a) whether 6.8's cap is genuinely awkward through `zip`'s API and (b) a `cargo geiger` measurement of the `unsafe` / surface actually removed (much of it is `miniz_oxide`, which we keep regardless). If vendored, add a `fuzz_zip` target so the new surface is covered from day one — owning code is a security win only when it shrinks the *fuzzed* trusted surface, not merely the dependency list; replacing a mature, fuzzed inflate with hand-rolled code would *raise* risk.
...
**Deliverable:** `anamnesis` v0.6.3 — the security core (caller-configurable `ParseLimits` per-item + aggregate, a decompression-ratio cap, the documented inspect-before-parse policy gate) **plus** the `remember_to_bytes` in-memory dequant companion (a non-security `Added` that rides in the same tag so we don't ship a one-method release). Default behaviour unchanged; no breaking change. The `CHANGELOG` carries **both** a `### Security` section (the governance work) and an `### Added` section (`remember_to_bytes`), keeping the security narrative honest. Lands before Phase 7 so `pip install anamnesis` exposes an already-governed parser. — **PUSH + tag `v0.6.3`**
