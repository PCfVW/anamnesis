...
- [3. Phased Development Plan](#3-phased-development-plan)
  ...
  - [Phase 6.10: Pickle-VM Working-Set Governance](#phase-610-pickle-vm-working-set-governance)
  - [Phase 6.11: Vendored ZIP Container Reader](#phase-611-vendored-zip-container-reader)
...
**Sequencing — three governance pieces, then tests/fuzz + the audit's one non-security nit, one patch version (`v0.6.5`):**

- [ ] **Step 1 — Always-on structural cap:** a permanent max stack-depth / value-count enforced *inside* `execute()` (the `MAX_PKL_SIZE`-style permanent floor), bounding (a) even under `ParseLimits::default()`. — **commit**
- [ ] **Step 2 — Wire the working set to `Budget`:** route every value-push through one `push_value` choke point that charges the value's heap cost, and charge the *deep* size on `BINGET` / `MEMOIZE` clones — bounding (b) and giving `max_total_bytes` real teeth over the VM. — **commit**
- [ ] **Step 3 — Nesting-depth cap during `execute()`** (and/or an iterative `Drop` for `PickleValue`): bound (c) and make recursive `Drop` stack-safe — needed independently of the budget because the default is unbounded. — **commit**
- [ ] **Step 4 — Regression tests + fuzz + the BnB nit:** assert each construction (N-flood, memo-replay, `Q`-chain) returns a clean `Err` *quickly* — not a runnable repro, since under `panic = "abort"` a successful repro self-terminates the test process; a real WSL `fuzz_pth*` campaign, ideally a structure-aware pickle target; and fold in the audit's defense-in-depth nit — BnB rejects an odd `block_size` (currently accepted → wrong output, not OOB). — **commit** — **PUSH + tag `v0.6.5`**
...
**Sequencing — one patch version (`v0.6.6`):**
...
- [ ] **Step 3 — Drop the `zip` runtime dependency** (move to `[dev-dependencies]`); measure the metadata reduction with `dhat` against the 50 000-tiny-entry fixture (projected ~12× from the measured ~500 → ~40 B/entry — confirm here) and record before/after in the commit. — **commit** — **PUSH + tag `v0.6.6`**

**Deliverable:** `anamnesis` `v0.6.6` — `.pth` / `.npz` container parsing on a lean owned reader (metadata reduction measured and recorded — projected ~12×), the `zip` crate off the runtime path, `miniz_oxide` retained for inflate, `fuzz_zip` shipped. No public API or behaviour change for legitimate files. — **PUSH + tag `v0.6.6`**
