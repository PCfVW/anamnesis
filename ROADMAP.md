...
- [x] **Step 5 — `cargo fuzz` harness:** a `fuzz/` cargo-fuzz workspace member (excluded from the published crate via `exclude = ["/fuzz"]`; nightly + libFuzzer, not in the stable CI matrix) with one target per parser (`fuzz_safetensors` / `fuzz_npz` / `fuzz_gguf` / `fuzz_pth`). libFuzzer doesn't build on Windows-MSVC, so it was **authored and verified under WSL2 Ubuntu** (nightly `rustc` + `cargo-fuzz` 0.13): all four compile; smoke campaigns ran ~20 s unseeded each (~0.2–0.8 M runs) plus a 62 s seeded `fuzz_pth` run from the AlgZoo fixtures (213 k runs, cov 1412 / ft 5372, corpus 675, RSS steady ~489 MB) — **zero crashes**. The split valve was not needed. A scheduled Linux fuzz CI job is a candidate follow-up. — **commit `b7499ea`**
