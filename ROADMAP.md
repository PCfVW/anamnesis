...
- [3. Phased Development Plan](#3-phased-development-plan)
  ...
  - [Phase 6.9: Pickle-VM Working-Set Governance](#phase-69-pickle-vm-working-set-governance)
  - [Phase 6.10: Vendored ZIP Container Reader](#phase-610-vendored-zip-container-reader)
...
**Sequencing — three governance pieces, then tests/fuzz + the audit's one non-security nit, one patch version (`v0.6.4`):**
...
**Sequencing — one patch version (`v0.6.5`):**
...
- [ ] **Step 3 — Drop the `zip` runtime dependency** (move to `[dev-dependencies]`); measure the metadata reduction with `dhat` against the 50 000-tiny-entry fixture (projected ~12× from the measured ~500 → ~40 B/entry — confirm here) and record before/after in the commit. — **commit** — **PUSH + tag `v0.6.5`**

**Deliverable:** `anamnesis` `v0.6.5` — `.pth` / `.npz` container parsing on a lean owned reader (metadata reduction measured and recorded — projected ~12×), the `zip` crate off the runtime path, `miniz_oxide` retained for inflate, `fuzz_zip` shipped. No public API or behaviour change for legitimate files. — **PUSH + tag `v0.6.5`**
