...
**Deliverable:** `anamnesis` `v0.6.7` — `.pth` / `.npz` container parsing on a lean owned reader (metadata reduction measured and recorded — projected ~12×), the `zip` crate off the runtime path, `miniz_oxide` retained for inflate, `fuzz_zip` shipped. No public API or behaviour change for legitimate files. — **PUSH + tag `v0.6.7`**
