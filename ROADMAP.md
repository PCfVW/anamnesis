...
- [ ] **Step 1b — Cross-architecture plain-FP4 fixture (Qwen3)** — download `ema1234/qwen_mcqa_bnb_fp4` via `hf-fm`, extend `generate_bnb.py` with a Qwen3 entry, add `cross_validate_encode_qwen3_mcqa_fp4` to `cross_validation_bnb_encode.rs`. Proves the sign-of-zero preservation rule (introduced in 1a for the Llama `FP4` fixture) generalises to a different architecture and a different HF org. ~550 MiB download, ~3 effort units, **commit**.

- [ ] **Step 1c — `encode_bnb4_double_quant` + cross-architecture DQ NF4 fixtures (Qwen2.5 + Phi-3.5)** — implement the fourth encode kernel (nested-absmax encode + nested-codebook search via `nearest_codebook_index_256`, mirror of the existing `dequantize_bnb4_double_quant_to_bf16`), validated against the existing `medmekk/Llama-3.2-1B-Instruct-bnb-nf4-double-quant` fixture's decode→encode round-trip *first*. Then download `unsloth/Qwen2.5-1.5B-Instruct-bnb-4bit` and `unsloth/Phi-3.5-mini-instruct-bnb-4bit` (both confirmed double-quant via `hf-fm inspect` before download), extend `generate_bnb.py` with both entries, add two new cross-validation tests. **Why de-deferred from polish to required** (see "Ecosystem finding" below). ~300 LOC + ~3.8 GiB download, ~6 effort units, **commit** — **PUSH + tag `v0.5.0`**.
