...
- **Validation infrastructure (Phase 6.5)**: anamnesis ships dev-only `criterion` runtime benchmarks (`benches/dequant.rs`, `benches/parsing.rs`) AND `dhat-rs`-instrumented peak-heap regression tests (`tests/peak_heap_{gptq,awq,bnb_dq}.rs`) that assert observed allocations stay within the documented `# Memory` ceilings. Every kernel's observed scratch matches the documented claim **to the byte** on the reference machine. No other crate in the matrix exposes both perf and memory-efficiency claims as reproducible-from-the-tree assertions — `mistralrs` carries internal benchmarks but they're inference-coupled; `gguf-rs` / `safetensors_explorer` / `apr-cli` ship no perf-validation infrastructure visible in their repos.
