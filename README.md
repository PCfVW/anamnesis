...
**v0.6.6** governs the `.pth` pickle VM's *working set* (Phase 6.11), closing a P0 an independent audit surfaced after v0.6.3. The earlier caps bounded the `data.pkl` opcode-stream length and individual string/bytes payloads, but the VM's **value stack, memoised clones, and nesting depth were charged to nothing** — so a small crafted pickle could amplify into multi-GiB heap (a `NONE`-flood, or `BINGET` replay of a large memoised subtree — a *separate* allocation per opcode/clone with no bound on the **total**, [CWE-1325](https://cwe.mitre.org/data/definitions/1325.html)) or a recursive-`Drop` stack overflow on a deeply-nested value ([CWE-674](https://cwe.mitre.org/data/definitions/674.html)), aborting the process (the crate builds `panic = "abort"`) even on the cheap `inspect_pth_from_reader` pre-filter the recipe above recommends for untrusted input. (Both sit under the [CWE-770](https://cwe.mitre.org/data/definitions/770.html) / [CWE-400](https://cwe.mitre.org/data/definitions/400.html) umbrella the header-allocation caps above also address, but the amplification and recursion shapes are distinct from a single oversized field.) Every value-creating opcode now flows through one `O(1)`-per-opcode accounting choke point: each pushed value and the deep size of each memo clone is charged to a permanent **512 MiB working-set floor** *and* the caller's `max_total_bytes`, and construction nesting is capped at a permanent **depth of 256** so an over-deep value never forms (recursive `Drop` and every recursive walk stay shallow). Both floors are always-on — they bite even under `ParseLimits::default()` — and the accounting is `O(1)` per opcode by design (a per-push deep walk would itself be an `O(n²)` CPU-DoS). A 385k-run RSS-limited `fuzz_pth` campaign runs clean. The same audit prompted a bundled defence-in-depth fix: BnB4 dequant/encode now reject an odd `block_size` (which truncated `bytes_per_block = block_size / 2` into mis-aligned output). No API change; honest files are unaffected.

**v0.6.7** replaces the `zip` crate's container parser with a lean, vendored, **read-only** central-directory reader (Phase 6.12), closing a *bounded* metadata-amplification gap a measurement reopened from the v0.6.3 audit. `zip::ZipArchive::new` eagerly materialises the whole central directory into a fat per-entry record — **measured at 337 B/entry** on a 50 000-tiny-entry archive — versus the **~40 B/entry** anamnesis actually needs (a `name → (offset, size)` index); that amplification is bounded (entries are file-grounded, so it sits under the [CWE-770](https://cwe.mitre.org/data/definitions/770.html) / [CWE-400](https://cwe.mitre.org/data/definitions/400.html) umbrella but is a constant-factor cost, *not* the unbounded v0.6.6 DoS) yet is neither reachable through `zip`'s API nor bounded by `ParseLimits` — only an owned reader bounds it. The vendored reader owns just the **container** (end-of-central-directory + central-directory + local-header offsets, with full **ZIP64** support so multi-GiB / >65 535-entry `torch.save` checkpoints keep parsing); `DEFLATE` `.npz` entries still inflate through `flate2` / `miniz_oxide` (the upstream-fuzzed codec surface is untouched). It is hardened to the same "untrusted input" bar as every other parser: bounds-checked cursor reads, `checked_*` offset arithmetic, a permanent entry-count cap and per-name cap *before* allocation, a `data_start + size ≤ file_len` cross-check, and a compression **allowlist**. Crucially, the owned reader *closes* the `ParseLimits` gap that motivated it: `read_central_directory` now consults the caller's `ParseLimits`, rejecting an over-budget declared entry count or central-directory allocation fail-fast (the inspect paths stay deliberately limit-free; the permanent floor still applies), and on the mmap path the directory is read zero-copy from the mapping. A self-review also bounded `DEFLATE` inflation on the `.pth` reader path with `Read::take(uncompressed_size)` so a crafted compressed `data.pkl` cannot zip-bomb past its declared, capped size ([CWE-409](https://cwe.mitre.org/data/definitions/409.html)). The `.pth` entry index moved from a `HashMap<String, …>` to a sorted, `shrink_to_fit`-trimmed `Vec<(Box<str>, …)>`, taking resident container metadata to **41 B/entry — an 8.07× reduction** (3.12× on peak; `tests/peak_heap_zip_metadata.rs`, `dhat`). `zip` leaves the runtime dependency tree entirely (now a dev-dependency for the `ZipWriter` test fixtures and as the **differential oracle** every parse is checked against), a new `fuzz_zip` target lands alongside, and there is no public API or behaviour change for legitimate files.

**v0.6.8** hardens the parser surface for the coming Python bindings (Phase 6.13) — library-side, so every Rust consumer benefits too. A **copy-based, mmap-free full-parse path** (`parse_bytes` / `parse_*_from_reader`) becomes the recommended entry point for untrusted input: it reads into an owned `Vec<u8>` rather than memory-mapping, so a truncated or concurrently-written source is a clean `Err` instead of an **uncatchable `SIGBUS` / `SIGSEGV`** — the OS signals a Python `try/except` cannot catch (the mmap `parse()` fast path stays for trusted local files). `AnamnesisError` gains a **finer taxonomy** — `LimitExceeded { limit }` and `DisallowedGlobal { module, name }` split out of the formerly-opaque `Parse` — with a frozen Rust→Python exception map, so a multi-tenant host can route a budget/cap breach (*413*), a malformed file (*400*), and a non-`torch.*` pickle global (a security signal) on the error *kind* rather than a string match; both are reachable even under `ParseLimits::default()` (the allowlist and the permanent floors are always-on). **Panic/abort-freedom** becomes a *tested* invariant — a stable-CI `catch_unwind` battery over every public parse/inspect entry point, plus owned-path `cargo fuzz` targets (~76 M executions, zero crashes) — and the Phase 8 PyO3 `cdylib` is pinned to a `panic = "unwind"` profile (`tests/panic_profile.rs`) so a panic surfaces as a catchable `PanicException` instead of killing the worker, while the library / CLI keep `panic = "abort"`. Finally, the **`NumPy` / `BF16` data-ownership contract** is locked ([`docs/python-interop.md`](docs/python-interop.md)): returned arrays own their bytes by default, so no array can alias a `Backing` the owning `Parsed*` drops (the use-after-free reachable from pure Python is structurally impossible), and `BF16` returns an `ml_dtypes.bfloat16` array when that optional dep is present, else raw `bytes` + a `"bfloat16"` dtype string — never a silent upcast. No behaviour change for existing mmap-path callers, and no public API removed.
